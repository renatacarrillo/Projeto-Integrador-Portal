<!DOCTYPE html>
<html lang="pt-br">
<!--importando o head da pagina -->
<?php
    include('../imports/head.php');
  ?>

<body>
    <!--importandoo menu da pagina -->
    <?php
    include('../imports/menu-secundario.php');
  ?>
    <main>
        <div class="container">
            <div class="row">
                <div class="col-sm-12">
                        <h1 class="text-white bg-dark text-center font-italic font-weight-bold">Mais Ouvidas</h1>
                </div>
            </div>
<div class="row">
<div class="col-sm-12">
            <table class="table table-striped table-dark">
                <thead>
                    <tr>
                        <th scope="col">Posição</th>
                        <th scope="col">Artista</th>
                        <th scope="col">Música</th>
                        <th scope="col">Albúm</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <th scope="row">1</th>
                        <td>Drake</td>
                        <td>Nonstop</td>
                        <td>Scorpion</td>
                    </tr>
                    <tr>
                        <th scope="row">2</th>
                        <td>Drake</td>
                        <td>Don’t Matter To Me ft Michael Jackson</td>
                        <td>Scorpion</td>
                    </tr>
                    <tr>
                        <th scope="row">3</th>
                        <td>Drake</td>
                        <td>In My Feelings</td>
                        <td>Scorpion</td>
                    </tr>
                    <tr>
                        <th scope="row">4</th>
                        <td>Drake</td>
                        <td>No Emotions</td>
                        <td>Scorpion</td>
                    </tr>
                    <tr>
                        <th scope="row">5</th>
                        <td>Drake</td>
                        <td>I’m Upset</td>
                        <td>Scorpion</td>
                    </tr>
                    <tr>
                        <th scope="row">6</th>
                        <td>Drake</td>
                        <td>Survive</td>
                        <td>Scorpion</td>
                    </tr>
                    <tr>
                        <th scope="row">7</th>
                        <td>Drake</td>
                        <td>Elevate</td>
                        <td>Scorpion</td>
                    </tr>
                    <tr>
                        <th scope="row">8</th>
                        <td>Drake</td>
                        <td>Mob Ties</td>
                        <td>Scorpion</td>
                    </tr>
                    <tr>
                        <th scope="row">9</th>
                        <td>Drake</td>
                        <td>God’s Plan</td>
                        <td>Scorpion</td>
                    </tr>
                    <tr>
                        <th scope="row">10</th>
                        <td>Drake</td>
                        <td>8 Out Of 10</td>
                        <td>Scorpion</td>
                    </tr>
                </tbody>
            </table>
    </div>
</div>

            <div class="container-fluid">
                <div class="row">
                    <div class="col-sm-2 col-xs-2">
                        <h4 class="text-center">Nonstop</h4>
                        <img src="../images/hip-hop/capadrake.jpg" class="img-fluid espa" alt="Responsive image">
                        <button type="button" class="btn btn-primary btn-lg btn-block btn btn-success float-none">Spotify</button>
                    </div>
                    <div class="col-sm-2 col-xs-2">
                        <h4 class="text-center">God's Plan</h4>
                        <img src="../images/hip-hop/capadrake.jpg" class="img-fluid espa" alt="Responsive image">
                        <button type="button" class="btn btn-primary btn-lg btn-block btn btn-success float-none">Spotify</button>
                    </div>
                    <div class="col-sm-2 col-xs-2">
                        <h4 class="text-center">Elevate</h4>
                        <img src="../images/hip-hop/capadrake.jpg" class="img-fluid espa" alt="Responsive image">
                        <button type="button" class="btn btn-primary btn-lg btn-block btn btn-success float-none">Spotify</button>
                    </div>
                    <div class="col-sm-2 col-xs-2">
                        <h4 class="text-center">Mob Ties</h4>
                        <img src="../images/hip-hop/capadrake.jpg" class="img-fluid espa" alt="Responsive image">
                        <button type="button" class="btn btn-primary btn-lg btn-block btn btn-success float-none">Spotify</button>
                    </div>
                    <div class="col-sm-2 col-xs-2">
                        <h4 class="text-center">I'm Upset</h4>
                        <img src="../images/hip-hop/capadrake.jpg" class="img-fluid espa" alt="Responsive image">
                        <button type="button" class="btn btn-primary btn-lg btn-block btn btn-success float-none">Spotify</button>
                    </div>
                    <div class="col-sm-2 col-xs-2">
                        <h4 class="text-center">Elevate</h4>
                        <img src="../images/hip-hop/capadrake.jpg" class="img-fluid espa" alt="Responsive image">
                        <button type="button" class="btn btn-primary btn-lg btn-block btn btn-success float-none">Spotify</button>
                    </div>
                </div>
            </div>
        </div>
    </main>
    <!-- importando o footer da pagina -->
    <?php
    include('../imports/footer-secundario.php');
  ?>
</body>

</html>
